...
public class LecturerPanel extends JPanel {
    private JTextField txtStaffNum, txtName, txtEmail, txtSearch;
    ...
    public LecturerPanel() {
        ...
        txtStaffNum = createStyledTextField();
        txtName = createStyledTextField();
        txtEmail = createStyledTextField();

        addFormRow(formPanel, "Staff Number", txtStaffNum, gbc, 0);
        addFormRow(formPanel, "Full Name", txtName, gbc, 2);
        addFormRow(formPanel, "Email Address", txtEmail, gbc, 4);

        btnAdd = new JButton("Add New Lecturer");
        ...
        String[] columns = {"Staff Number", "Full Name", "Email", "Edit Action", "Delete Action"};
        ...
        table.setRowHeight(38); // Breathing room for the custom button outline badges
        ...
        // Appending the custom button renderers directly to the columns
        table.getColumnModel().getColumn(3).setCellRenderer(new ActionButtonRenderer(MINT_GREEN));
        table.getColumnModel().getColumn(4).setCellRenderer(new ActionButtonRenderer(ORANGE_RED));
        ...
        // ==========================================
        btnAdd.addActionListener(e -> saveLecturer());
        ...
        table.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                String staffNum = table.getValueAt(row, 0).toString();
                String name = table.getValueAt(row, 1).toString();
                String email = table.getValueAt(row, 2).toString();

                if (col == 3) { 
                    openEditModal(staffNum, name, email);
                } else if (col == 4) { 
                    int confirm = JOptionPane.showConfirmDialog(LecturerPanel.this, 
                            "Are you sure you want to delete lecturer " + staffNum + "?", 
                            "Confirm Deletion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                    if (confirm == JOptionPane.YES_OPTION) {
                        if (controller.deleteLecturer(staffNum)) {
                            JOptionPane.showMessageDialog(LecturerPanel.this, "Lecturer entry cleanly removed.");
                            refreshTableData("");
                        }
                    }
                }
    ...
    private void refreshTableData(String keyword) {
        ...
        List<Lecturer> lecturers = controller.getAllLecturers(keyword);
    ...
    private void saveLecturer() {
        Lecturer l = new Lecturer(0, txtStaffNum.getText().trim(), txtName.getText().trim(), txtEmail.getText().trim(), 0);
        if(controller.addLecturer(l)) {
            JOptionPane.showMessageDialog(this, "Lecturer added successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
            txtStaffNum.setText(""); txtName.setText(""); txtEmail.setText("");
            refreshTableData("");
        } else {
            JOptionPane.showMessageDialog(this, "Submission processing error.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void openEditModal(String staffNum, String name, String email) {
        JFrame topFrame = (JFrame) SwingUtilities.getWindowAncestor(this);
        JDialog editDialog = new JDialog(topFrame, "Modify Entry Record", true);
        editDialog.setSize(360, 250);
        editDialog.setLocationRelativeTo(this);
        editDialog.setLayout(new GridLayout(4, 2, 12, 12));
        ...
        JTextField mEmail = createStyledTextField(); mEmail.setText(email);
        ...
        JLabel lblStaffTitle = new JLabel("Staff Number:"); lblStaffTitle.setFont(new Font("Segoe UI", Font.BOLD, 12));
        JLabel lblStaffVal = new JLabel(staffNum); lblStaffVal.setFont(new Font("Segoe UI", Font.BOLD, 13));
        lblStaffVal.setForeground(ORANGE_RED);

        editDialog.add(lblStaffTitle); editDialog.add(lblStaffVal);
        editDialog.add(new JLabel("Full Name:")); editDialog.add(mName);
        editDialog.add(new JLabel("Email:")); editDialog.add(mEmail);
        editDialog.add(new JLabel("")); editDialog.add(btnUpdate);

        btnUpdate.addActionListener(e -> {
            Lecturer updated = new Lecturer(0, staffNum, mName.getText().trim(), mEmail.getText().trim(), 0);
            if (controller.updateLecturer(updated)) {
    ...
    private static class ActionButtonRenderer extends JPanel implements TableCellRenderer {
        ...
        public ActionButtonRenderer(Color themeColor) {
            ...
            // Custom pill-shaped border with internal padding margins
